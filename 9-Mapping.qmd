...
underground.coord <- undergroundRR %>% geocode(city = City, state = State, method='osm', lat= latitude, long = longitude)
```
...
head(BostonWomenVoters$Present.Residence)
bwvprep <- BostonWomenVoters %>%
  mutate(full.address = paste(Street.Number.on.April.1, Street.of.Residence.on.April.1, "Boston", "MA")) %>%
  drop_na(Ward) %>%
  slice(1:100)

bwvcoord <- bwvprep %>% geocode(address = full.address, method='osm', lat = latitude, long = longitude)
```
...
> This package includes county maps and state maps, as well as maps of France, Italy, New Zealand, the US, and 2 world maps.
...
> The longitude and latitude points that make up the borders of the states in the US, the group and order columns, and the region column, which includes the state. It also includes a subregion column that has the counties if you look at the county map.
...
```{r}
gayguides <- gayguides %>% filter(state %in% c("AL", "AZ", "AR", "CA", "CO", "CT", "DE", "DC", "FL", "GA", "ID", "IL", "IN", "IA", "KS", "KY", "LA", "ME", "MD", "MA", "MI", "MN", "MS", "MO", "MT", "NE", "NV", "NH", "NJ", "NM", "NY", "NC", "ND", "OH", "OK", "OR", "PA", "RI", "SC", "SD", "TN", "TX", "UT", "VT", "VA", "WA", "WV", "WI", "WY"))

ggplot() + 
  geom_map(data = usa, map = usa, aes(long, lat, map_id=region), fill = "lightgray", color = "black") +
  geom_point(data = gayguides, mapping = aes(x = lon, y = lat, color = state), size = 3) + coord_fixed(1.3) + 
  labs(title = "Gay Friendly Locations in the US in 1965")
```

(@) Can you map just locations in South Carolina (on a basemap of SC)? 
```{r}
data(gayguides)
gayguidessc <- gayguides %>% filter(state == "SC")
scmap <- usa %>% filter(region == "south carolina")

ggplot() + 
  geom_map(data = scmap, map = scmap, aes(long, lat, map_id=region), fill = "lightgray", color = "black") +
  geom_point(data = gayguidessc, mapping = aes(x = lon, y = lat), color = "red") + coord_fixed(1.3)
```

(@) Create a map that uses your geocoded data from the Boston Women Voters dataset. 
```{r}
library(maps)
boston <- map_data("county") %>% filter(region == "massachusetts" & subregion == "suffolk")

ggplot() +
  geom_map(data = boston, map = boston, aes(long, lat, map_id=region), fill = "lightgray", color = "navy") +
  geom_point(data = bwvcoord, mapping = aes(x = longitude, y = latitude, color = factor(Ward)), size = 1) + coord_fixed(1.3) +
  labs(title = "Women Voters in Boston by Ward")
```

```{r}
ma <- map_data("state") %>% filter(region == "massachusetts")

ggplot() +
  geom_map(data = ma, map = ma, aes(long, lat, map_id=region), fill = "lightgray", color = "navy") +
  geom_point(data = bwvcoord, mapping = aes(x = longitude, y = latitude, color = factor(Ward)), size = 1) + coord_fixed(1.3) +
  labs(title = "Women Voters in Boston by Ward")
```
...
bwvcoord <- bwvcoord %>% 
  arrange(longitude) %>%
  filter(latitude >= 42.2875)
```

#The state map holds everything, but I don't know if the boundaries were redone at one point and that's why some of my points are technically outside Boston.
...
ggplot() + 
  geom_map(data = usa, map = usa, aes(long, lat, map_id=region), fill = "lightgray", color = "black") +
  geom_point(data = rec.data.coordinates, mapping = aes(x=longitude, y=latitude)) + coord_fixed(1.3)
```

```{r}
ggplot() + 
  geom_map( data = usa, map = usa, aes(long, lat, map_id=region), fill="white", color="gray") +
  geom_point(data = rec.data.coordinates, mapping = aes(x=longitude, y=latitude, size=total_expenditures)) + coord_fixed(1.3)
```
...
> Including Alaska and Hawaii may make the map look weird because it has to stretch the map to include those points. By filtering to the contiguous 48 states, the map looks more normal and is easier to see points (to me at least). Obviously if a dataset includes points in Hawaii and Alaska, you wouldn't want to filter those two places out. I could see a scenario where you're potentially mapping networks of British-American communications related to the acquisition of Alaska being a reason to not filter that out. However, stretching the map looks like it would collapse multiple points in the northeast together because they are close together, which could cause someone to interpret influence in the northeast being more important than it actually is.
...
```{r}
percentage.foreign.male <- total_male_pop %>%
  mutate(percentforeignmale = (AZF003/total_male_pop))
```
...
> This code tells the computer to multiply the valus in the percentforeignmale column to be percentages and use those values to make up the scale.

Now create the map: 
```{r}
library(scales)
ggplot(data = percentage.foreign.male, aes(fill = percentforeignmale)) +
  geom_sf() + scale_fill_continuous("Percentage Foreign Born Male Population", labels = scales::percent)
```
...
```{r}
leaflet(data=bwvcoord) %>% addTiles() %>% addMarkers(~longitude, ~latitude, popup = paste("This woman's occupation was ", bwvcoord$Occupation, ".", sep=""))
```
...
> I chose to use the Boston Women's Voters dataset and map the occupation of the women. I could see leaflet being more useful for exploratory maps that you want to click on for more information (or if you have a bunch of points that are layered on top of one another), or if you need to map a lot of information in each point. I would use ggplot to map things for comparison, particularly using the facet wrap option to see changes in different groups, over time, etc.
...
```{r}
library(usmap)
usmap <- map_data("usmap")
parks <- read.csv("https://raw.githubusercontent.com/regan008/DigitalMethodsData/main/raw/parks-geocoded.csv")
```

```{r}
parks_west <- c("Washington", "Oregon", "Montana", "Wyoming", "Idaho", "Nevada", "California", "Utah", "Colorado", "Arizona", "New Mexico", "Hawaii", "Alaska")
parks_midwest <- c("North Dakota", "South Dakota", "Nebraska", "Kansas", "Iowa", "Minnesota", "Missouri", "Wisconsin", "Michigan", "Illinois", "Indiana", "Ohio")
parks_south <- c("Texas", "Oklahoma", "Arkansas", "Louisiana", "Mississippi", "Alabama", "Tennessee", "Kentucky", "West Virginia", "Virginia", "Georgia", "South Carolina", "North Carolina", "Florida", "Maryland", "Delaware")
parks_northeast <- c("Pennsylvania", "New Jersey", "New York", "Vermont", "New Hampshire", "Massachussetts", "Rhode Island", "Connecticut", "Maine")

southern.parks <- parks %>%
  filter(state_region_province %in% parks_south)
```

```{r}
grouped.parks <- parks %>%
  mutate(region = case_when(
    state_region_province %in% parks_west ~ "west",
    state_region_province %in% parks_midwest ~ "midwest",
    state_region_province %in% parks_south ~ "south",
    state_region_province %in% parks_northeast ~ "northeast")) %>%
    filter(country == "United States")

ggplot() + 
  geom_map(data = usa, map = usa, aes(long, lat, map_id=region), fill = "lightgray", color = "black") +
  geom_point(data = grouped.parks, mapping = aes(x=lon, y=lat)) + coord_fixed(1.3)

plot_usmap(regions = "states") + labs(title = "USA Map", subtitle = "AK and HI included")
# this map doesn't have Alaska and Hawaii, but I can't figure out how to fix this
```

```{r}
parks.map <- leaflet()
parks.map %>% addTiles()
leaflet(data=parks) %>% addTiles() %>% addMarkers(~lon, ~lat, popup = paste("This park is ", parks$park, ". ", "The closest city is ", parks$closest_city, ", which is ", parks$distance_from_closest_city, " km away .", sep=""))
```