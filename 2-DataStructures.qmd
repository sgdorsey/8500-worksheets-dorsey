...
```{r}
oldvoters<-BostonWomenVoters[BostonWomenVoters$Age >= 50, ]
youngvoters<-BostonWomenVoters[BostonWomenVoters$Age < 50, ]
```

The age distribution of women voters in Boston was skewed toward younger voters, with 8119 of the voters being under 50 years old and only 2658 of the voters being 50 and older.
...
```{r}
mainst<-BostonWomenVoters[grepl("Main", BostonWomenVoters$Street.of.Residence.on.April.1), ]
```
Only 4 women lived on Main st.
...
head(womenorder)
```
The five oldest womens' occupations are as follows: "At home," "At home," "[home?]," "Housekeeper," and "Widow/nRetired"