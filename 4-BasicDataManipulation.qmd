...
> I put the information into a variable because selecting the city and state (without pasting them into a new column where they were combined) would already have pulled information in the database format.
...
```{r}
nolocation <- gayguides %>% select(-city,-state)
```
...
```{r}
gayguides %>% filter(Year == "1975", city != "New York City" & city != "San Francisco")
```
...
```{r}
lesbians <- gayguides %>% filter(grepl("(G)", gayguides$amenityfeatures) | grepl("(L)", gayguides$amenityfeatures))
```
...
```{r}
gayguides %>%
  mutate(city_title = str_to_title(city)) %>%
  select(city, city_title) %>%
  head()
```
...
```{r}
gayguides <- gayguides %>%
  mutate(has_dancing = grepl("(D)", gayguides$amenityfeatures))
```
...
```{r}
BostonWomenVoters %>% 
  mutate(Occupation, str_trim(BostonWomenVoters$Occupation, side = "both"))
```

```{r}
BostonWomenVoters <- BostonWomenVoters %>% 
  mutate(Occupation = str_trim(BostonWomenVoters$Occupation))
```
...
> It could have been illegible or not taken at all, which means that the historical record probably has gaps we should investigate further. 
...
```{r}
(sum(is.na(BostonWomenVoters$Ward))) + (sum(is.na(BostonWomenVoters$Age)))
BostonWomenVoters %>% 
  drop_na(Ward, Age) %>%
  nrow()
```

117 rows are removed.
...
```{r}
BostonWomenVoters <- BostonWomenVoters %>%
  mutate(ward_missing = is.na(BostonWomenVoters$Ward))
```
...
```{r}
class(gayguides$type)

gg <- gayguides %>%
  mutate(type_factor = as.factor(type))

levels(gg$type_factor)

```

There are 235 different types in the Gay Guides dataset.

(@) In the Boston Women Voters data, convert Occupation to a factor. What are the levels?
```{r}
class(BostonWomenVoters$Occupation)

bwv <- BostonWomenVoters %>%
  mutate(occ_factor = as.factor(Occupation))

levels(bwv$occ_factor)
```

There are 1137 levels in the BostonWomenVoters dataset.
