...
```{r}
presidents <- c("Madison", "Lincoln", "FDR", "Jefferson")
...
> I knew I had to find the max year, so I knew I needed to subset the gayguides with the Year. Then I decided to write the if/else code with the max year over 1990.
...
> Length gives the amount of items in the list. Using it on presidents will give the amount of entries in the string vector. This code works because the length([presidents) essentially equals 3. 
...
```{r}
classes <- c("History 8520", "History 8510", "History 8540")
for (p in 1:length(classes)) {
  print(paste("I am enrolled in", classes[p]))
}
```
...
```{r}
for (i in 1:nrow(gayguides)){
  if(grepl("New York City", gayguides$city[i])){
    print(gayguides$title[i])
  }
}
```
...
The loop only printed 3 and 4 because i<3 told it to skip 1 and 2, and break after i>=5 told the loop to stop once it reached any number that was 5 and above. The only numbers left were 3 and 4.
...
```{r}
characternumber <- function(mystring) {
...
>I knew that nchar would need to be used at some point, since that function would count the characters in a string. Then I duplicated the setup of the question about creating the greet() function, but I just substituted the nchar for the name.
...
>George Washington's grade's output gets stored in a variable, because if you view GeorgeWashington then you can see the output was "C." There's no command to print(GeorgeWashington), which would give you the grade.
...
```{r}
ggsclocations <- function(d.state, d.year) {

  for (i in 1:length(gayguides$state)) {
    if (gayguides$state[i] == d.state && gayguides$Year[i] == d.year) {
      print(paste("Found a location in", d.state, "called", gayguides$title[i]))
    } else {
      next
    }
  }
...
```{r}
bwvoccupations <- function(occupation) {
  result <-data.frame()
...
bwvoccupations("Housewife")

bwv.occupation.result <- bwvoccupations("Housewife")
```
...
```{r}
found_count <- 0
count_by_state <- function(state){

for (i in 1:nrow(gayguides)){
  if(gayguides$state[i] == state){
  found_count <- found_count + 1
  }
}
print(paste("There are", found_count, "locations in", state))
}
...
```{r}
found_count <- 0
count_by_state <- function(state, d.year){

for (i in 1:nrow(gayguides)){
  if(gayguides$state[i] == state && gayguides$Year[i]==d.year){
  found_count <- found_count + 1
  }
}
print(paste("There are", found_count, "locations in", state, "for the year", d.year))
}

count_by_state("SC", 1975)
```
...
>Loops and conditionals can all be part of functions. For example, in the last question, I created a function where each line of the function was tested to see if it met the condition of being in South Carolina in 1975. This function looped through all the entries of the data in the dataset.
