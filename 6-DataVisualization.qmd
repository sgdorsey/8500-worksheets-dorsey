...
> This data tracks the life expectantcy, population, and GDP per capita for each country for every 5 years beginning in 1952 until 2007.
...
#ggplot(data = gapminder) = tells ggplot to use the gapminder dataset
#mapping=aes(x = lifeExp, y = pop) = tells ggplot to map life expectancy as the x-axis and population as the y axis
#geom_point() = chooses a scatterplot
```
...
ggplot(data = rec, mapping = aes(x = population, y = total_expenditures)) + geom_point()

ggplot(data = rec, mapping = aes(x = total_expenditures, y = population)) + geom_point()
...
```{r}
ggplot(BostonWomenVoters, aes(x = Age)) + geom_histogram(bins = 50)
```

(@) Explain, what does changing the number of bins do? 

> Changing the number of bins changes how many ages are in each grouping. If the number of bins is too high, there are gaps and you can't see the trends in the data. If the number the bins is too low, you can't see any unique or rage trends in the data at all.
...
```{r}
data(gayguides)
carolinaga <- gayguides %>% filter(state %in% c("SC", "GA", "NC", Year=1980)) %>% group_by(state) %>% summarize(count = n())
ggplot(carolinaga, aes(x = state, y = count)) + geom_col()
```

(@) Using the `gayguides` data, plot the number of religious institutions in each state.
```{r}
religousgg <- gayguides %>% filter(grepl("religious", gayguides$type, ignore.case = TRUE)) %>% group_by(state) %>% summarize(count = n())
ggplot(religousgg, aes(x = state, y = count)) + geom_col()
```
...
< What were the three states with the most "Raunchy Patron" locations, and what was the distribution of those states' locations numerically?

(@) Now write the code to answer that question and use comments (`#`) to explain your thinking as you go.
```{r}
rp <- gayguides %>% filter(grepl("(RT)", gayguides$amenityfeatures)) %>% group_by(state) %>% summarize(count = n()) %>% head(3)
#wrote the code to filter the gayguides dataset by finding all instances of RT (the code for Raunchy Patrons) in the amenity features column, which negated having to split the column apart. Then I grouped it by state, summarized by count, and pulled the top 3 counts using the head() command
ggplot(data = rp, mapping=aes(x=state, y=count, fill=state)) + geom_col()
#set up the bar chart by giving the ggplot the data in the rp variable, mapping the state as the x axis and the count of the locations as the y axis, made the bars on the states different colors with the fill option, then told ggplot to create a bar chart
```
...
```{r}
gg.religious <- gayguides %>% filter(grepl("religious", gayguides$type, ignore.case = TRUE)) %>% group_by(Year) %>% summarize(count = n())
ggplot(gg.religious, aes(x = Year, y = count)) + geom_line()
```
...
```{r}
gg.risk <- gayguides %>% filter(grepl("(HOT)|(AYOR)", gayguides$amenityfeatures) | grepl("inquire locally", gayguides$description) | grepl("inquire locally", gayguides$streetaddress)) %>% group_by(Year) %>% summarize(count = n())
ggplot(gg.risk, aes(x = Year, y = count)) + geom_line()
```
...
```{r}
reccity <- rec %>% filter(state == "SC") %>% group_by(city, year, total_expenditures) %>% summarize(count = n())
ggplot(reccity, aes(x = year, y = total_expenditures)) + geom_col() + facet_wrap(~city)
```

(@) Using the gayguides data can you select 5 types of locations and chart the number of each type between 1965 and 1980? 
```{r}
#locations: "Hotels", "Baths", "Business,Erotic Shop", "Health Clinic", "Gym"
gg.loc <- gayguides %>% filter(type == "Hotels"| type == "Baths"| type == "Business,Erotic Shop"| type == "Health Clinic"| type == "Gym") %>% filter(Year >= 1965 & Year <= 1980) %>% group_by(type, Year) %>% summarize(count = n())
ggplot(gg.loc, aes(x = Year, y = count)) + geom_line() + facet_wrap(~type)

#turns out, I picked 5 location types and 3 of them didn't exist in this time frame.
```
...
rec.long <- rec.workers %>% pivot_longer(!city:type_of_worker, names_to = "year", values_to = "count")
rec.types2 <- rec.long %>% 
  mutate(
    type_of_worker = fct_collapse
    (type_of_worker,
      "paid" = c("female paid", "male paid"),
      "volunteer" = c("men volunteer", "volunteer", "women volunteer")
    )
  ) %>% 
  group_by(type_of_worker, year) %>% summarize(count = mean(count, na.rm = TRUE)) %>% mutate(year = as.numeric(gsub("X", "", year)))

ggplot(rec.types2, aes(x = year, y = count)) + geom_line() + facet_wrap(~type_of_worker)
```
...
```{r}
data(BostonWomenVoters)
southern <- c("Virginia", "South Carolina", "North Carolina","Louisiana", "Arkansas", "Mississippi", "Florida", "Alabama", "Texas", "Tennessee")
bwv.south <- BostonWomenVoters %>% filter(State.or.Province.of.Birth %in% southern) %>% group_by(State.or.Province.of.Birth) %>% summarize(count = n())
ggplot(bwv.south, aes(x = State.or.Province.of.Birth, y = count)) + geom_col()
```

(@) Plot 1 explanation: 
> This plot charts the number of women in the Boston Women Voters dataset who were born in a southern state (specifically one of the 11 states that seceded from the Union during the Civil War). I wanted to see where women who were originally from southern states were coming from - most women originally from southern states were from Virginia, followed by North Carolina and then South Carolina - since many of them would have either lived through the Civil War or been a direct descendant of someone who did. I wonder if this impacted the voting habits of these women one way or another.

(@) Plot 2
```{r}
data(almshouse_admissions)
grouped_almshouse <- almshouse_admissions %>% group_by(Year.Admitted) %>% summarize(count = n()) %>% filter(Year.Admitted >= 1793)
ggplot(grouped_almshouse, aes(x=Year.Admitted, y=count)) + geom_line() + geom_point()
```

(@) Plot 2 explanation: 
> This plot is a line plot with points showing the number of almshouse admissions over time. This line graph shows that almhouse admissions jumped significantly beginning in 1800 with a peak in 1803 (there were 442 admissions that year). I wanted to see if significant numbers of admissions corresponded with a major crisis in history (it didn't, but a Google search mentioned that Philadelphia was rebuilding from outbreaks of yellow fever in 1803).
...
```{r}
chsgroup <- charlestonManumissions %>% mutate(
    Race = fct_collapse
    (Race,
      "Black" = c("Black"),
      "Mixed" = c("Black, Mulatto, Mulatto", "Black/Mustizo", "Black; Mulatto", "Black; Mulatto; Mulatto", "Mulatto or Mustee", "Mestizo", "Mulatto", "Mustee", "Mustizoe"),
      "Moor" = c("Moor"),
    )
  ) %>% group_by(Race) %>% summarize(count = n())

ggplot(chsgroup, aes(x = Race, y = count)) + geom_point()
```

(@) Plot 3 explanation: 
> This dot plot shows the number of manumissions broken down by race, where racial categories included "Black," "Mixed Race," "Moor," a blank variable (which had 1 entry), and "NA." This plot shows that most of the manumissions were for Black people, followed closely by those who were a mix of races.
