...
```{r}
paste("This is my string", "and it is cool.")
```

(@) Can you explain what happened in 2-3 sentences? 

> The two separate strings were combined into one string. This one string is the output of the total command, which shows in the terminal.
...
>In 94, the code says that the statement 5=10 is false.
In 95, the code says that the statement 3<4 is true.
In 96, the code says 5 is not equal to 10 is a true statement.
In 97, the code says 5 is not equal to 5 is a false statement.
98 says that the statement "3 is equal to 4 or 3" is true.
99 says that the statement "3 is less than 4 and 3 is less than 5" is true.
...
```{r}
y <- "testvariable"
z <- "I couldn't remember what a string was"
```
...
>The code above took each number in the vector and multiplied it by two. Then it gave the multiplied numbers as the output in the R panel/terminal (I couldn't remember what we called it in class).
...
> The my.numbers[3] isolated the third number in the sequence (6). That number was multiplied by 2.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
cities <- c("Spartanburg", "Pelzer", "Greenville", "Pacolet")
cities
```
...
>floor is a rounding function that takes whatever is x and gives you the biggest integer that would not go over the number that was x. For example, if x was 5.1 and you did floor(5.1), the output would be 5. I was able to run the documentation, but I had to google it to understand it. 
...
```
What is going on here? Each line within this function (x,y,z) creates a new column and determines the vhalues in each row. So the first line creates five rows with values sequentially from 1 to 5. The second column contains only the value 1 in all five rows. 

(@) What is going on in row z? Can you explain the logic here?

>In row z, each number from 1 to 5 (which is each of the numbers for x) is being multuplied by 2 and then the y value (1) is added to it.
...
> This dataset contains 14 variables with 60698 observations (rows). The information included is the addresses in Bob Dameron's address books from 1965-1985.
...
>I remembered these commands from the Command Line Overview video, so I knew the head and tail commands. I figured if the head command worked in R the tail command would too.? I knew I had to define the number of rows for the command to pull from, but I couldn't figure out where to put the n=10, so I googled that. It does also show in the documentation.
...
>gayguides is the data set that's being looked at. The $ indicates that we're looking at one category or variable, which is the title. Then, the 500 indicates that the only thing that should be in the output is whatever is the title of the 500th place in the list. It continuously narrows the scope of the command to make it more specific.
...
min(gayguides$Year)
```
The earliest year is 1965.

(@) How about the latest year? 
```{r}
max(gayguides$Year)
```
The latest year is 1985.
...
min(almshouse_admissions$Year.Admitted, na.rm = TRUE)
max(almshouse_admissions$Year.Admitted)
max(almshouse_admissions$YearDischarged)
...
>This dataset has 9 variables and 1400 observations. The Almshouse Admissions data is a sample of the people who were admitted to the Philadelphia Almshouse between 1796 and 1803. The latest discharge date was in 1813. The latest admission date was in 1805. I could not get a year for the minimum year Admitted, as my command kept giving me 0 for the output. I tried to Google it and I tried adding na.rm = TRUE to remove any missing values. After posting this question in slack, I viewed the data in the environment and sorted the Year.Admitted column in ascending order, which showed that two rows did have 0 as the value in the Year.Admitted column. The dataset includes information such as gender, descriptions, the reason for discharge, and the names of the admitted.
