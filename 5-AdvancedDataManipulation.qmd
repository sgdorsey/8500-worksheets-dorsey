...
```{r}
avgagebyoccupation <- BostonWomenVoters %>%
  group_by(Occupation) %>%
  summarize(
...
```{r}
locgroup <- gayguides %>%
  filter(grepl("Bar", gayguides$type)) %>%
  group_by(type,state) %>%
  summarize(number = n()) %>%
  arrange((desc(number)))
```
California has the most bars, with 5490 entries.
...
southstates <- gayguides %>%
  filter(state %in% southernstates) %>%
  group_by(Year) %>%
  summarize(count=n()) %>%
  filter(Year == "1975")
```
There were 663 locations in all the southern states in 1975.
...
gayguides <- gayguides %>% mutate(State=paste(state))
gayguides <- left_join(gayguides, regions, by=c("State" = "State.Code"))
regionsgroup <- gayguides %>% group_by(Division.y) %>% summarize(numberregion = n())
```
There are 3607 locations in the mountain region.


(@) Explain what you did above. What variable did you join by and why? What results?
I joined the state column in the gayguides data with the state.code column in regions since both of them have an abbreviated state name ("SC"). That added the region, division, and a duplicate of the state name to the original gayguides dataset.
...
```{r}
yeargrowth <- gayguides %>%
  filter(Year=="1980" | Year=="1970") %>%
  group_by(state,Year) %>%
  summarize(count=n()) %>%
  mutate(Previous_Value = lag(count), count-Previous_Value)
```
```{r}
regiongrowth <- gayguides %>%
  filter(Year=="1980" | Year=="1970") %>%
...
> This code collapsed the years into one column, instead of having them across multiple columns. To do this, it duplicated the rows to account for the extra years.
...
>Waitress, widow (though that is often combined with something), unemployed, tutor, trained nurse.
...
>"Housewife," "At home," and "Housekeeper" are the three most common occupations.
...
```{r}
BostonWomenVoters %>%
  mutate(occupation_lumpedten = fct_lump(Occupation, n=10)) %>%
  count(occupation_lumpedten) %>%
  arrange(desc(n))
```
...
> The occupations with more than 20 people with the youngest average age were students, telephone operators, and school teachers. The oldest average age was found in people with no occupation, housekeepers, and "matron." Usually, it seems that the older women who were not working outside of the home or at all were voting, while younger women who worked outside of the home or were students were voting. It could give us insights into the classes of women who were voting, and it could also lead us to look further into the students to see if they were in womens' rights groups at school or if they may have had a teacher that was prominent in the women's rights movement.
...
test <- BostonWomenVoters %>%
  mutate(
    occupation_category = fct_collapse(Occupation,
    "Domestic.labor" = c("housewife", "Housewife", "At home", "At Home", "Housekeeper", "House keeper"),
    "Service" = c("waitress", "cashier", "laundress", "hairdresser", "Tailoress", "Janitress", "Manicurist"),
    "Technical" = c("operator", "telephone operator", "Tel. Operator"),
    "Financial" = c("Insurance", "Ledger clerk", "Accountant", "Billing clerk", "Bank Clerk", "saleswoman", "Book Keeper", "bookkeeper"),
    "Arts" = c("Artist", "Artist, single", "Pianist", "Actress", "musician", "Vaudeville")
    )
  ) %>%
  count(occupation_category) %>%
  arrange(desc(n))
...
Domestic labor would include everyone who is a housewife or works strictly in the home. The "service" category includes people who perform service jobs but may be outside of the home. Technical jobs were those that required knowledge of the technology of the time, particularly surrounding the telephone. Financial jobs included anyone who worked in financial sectors, which would have required some training and a good grasp of math, which in theory would have required schooling. Arts jobs were any that were in an artistic field. I thought it would be interesting historically to see it the arts had more women who were voting than housewives, for example.
...
BostonWomenVoters %>%
  mutate(
    occupation_category = fct_collapse(Occupation,
    "White Collar" = c("college executive", "Consulting Actuary", "Dentist", "Dep Collection U.S. Internal Revenue", "Dietitian"),
    "Pink Collar" = c("Teacher", "hairdresser", "clerk","cook", "laundress", "Manicurist"),
    "Blue Collar" = c("Forelady", "forewoman", "Lamp Inspector", "Factory work", "Factory Work"),
    ),
    occupation_category = fct_relevel(occupation_category,
...
> Explanation: I chose this order because if everything was assigned the correct category, I think it would be interesting to lay the 60s-70s divisions of what kinds of labor there are over the occupations women voters were holding in the 1920s. I think it would be interesting to see if most of the voters were of a certain type of work.
...
#updated from class March 9
dates_demo <- c("12/08/2001", "05/15/1983")
parsed_dates <- mdy(dates_demo)
```
...
```{r}
gayguides <- gayguides %>%
  mutate(Date = ymd(paste(gayguides$Year, "01", "01", sep = "-")))
```
...
(@) Using the dates you created above for `gayguides`, extract the decade from the date. Compare it to using `floor(Year / 10) * 10` directly on the Year column. Do you get the same result? - Yes
```{r}
gayguides <- gayguides %>%
  mutate(year = year(gayguides$Date),
        decade = (year %/% 10) * 10)

gayguides <- gayguides %>%
  mutate(yrfloor = floor(Year / 10) * 10)
```
...
BostonWomenVoters <- BostonWomenVoters %>%
  ...
  mutate(Date = ymd(paste(BostonWomenVoters$birthyear, "07", "01", sep = "-")))
```
...
for (i in 1:nrow(gayguides)) {
   if (ymd(gayguides$Date[i]) %within% sixties) {
      print("sixties")
   } else if (ymd(gayguides$Date[i]) %within% seventies) {
      print("seventies")
   } else if (ymd(gayguides$Date[i]) %within%eighties) {
      print("eighties")
   } else {
      print("n/a")
   }
...
#updated after class on March 9
gayguides <- gayguides %>%
  mutate(in_1960s = date %within% sixties)
```
Dr. Regan, please read this: I tried to save this to a variable and it wouldn't save to the variable, then it kept running this for a solid 2 minutes, so I left it alone after this.
...
fillyear <- function(year){
  statement <- ymd(paste0(year, "-01-01"))
  statement
}
fillyear(1920)
fillyear(1934)
...
> Sometimes dates aren't standardized, especially if different people record the dates differently. For example, in my data for church archives, one surveyor recorded dates as 05-12-36 and another recorded dates as 1936-05-12. Lubridate can help standardize these issues. In some cases, people may not have recorded all parts of data (some of my data only has the year, while others have months and years of churches being organized), which can be fixed by using lubridate in a function to add months and dates, if they aren't particularly important, so your data can be standardized.
...
```{r}
How many judges were appointed from South Carolina and which party was the president appointing them from?
```
...
```{r}
scjudges <- judges %>%
  filter(Birth.State == "SC") %>%
  group_by(Party.of.Appointing.President..1.) %>%
  summarize(count=n())
```

(@) Now ask a question of the `gayguides` data (or another dataset of your choice). What is the question you are asking? 
```{r}
What types of locations were in the address books during what years in SC?
```

(@) Now write some code to address that question. Comment the code with notes that explain your thinking as you go. Use functions like select(), filter(), etc to pair down your dataset and reshape it to address your question. 
```{r}
sctypes <- gayguides %>%
#filtered by state to pull everything from SC
  filter(state == "SC") %>%
  #grouped by type and year to see if the numbers change over time
  group_by(type, Year) %>%
  #summarized the count of the group_by output to see if the numbers changed.
  summarize(count = n())
```
...
```{r}
fourreturn <- function(d.Year, d.state){
  gayguides %>%
  filter(Year == d.Year & state == d.state) %>%
...
```{r}
bwvclean <- BostonWomenVoters %>%
  drop_na(Ward, Age) %>%
  mutate(
    occupation_category = fct_collapse(Occupation,
    "White Collar" = c("college executive", "Consulting Actuary", "Dentist", "Dep Collection U.S. Internal Revenue", "Dietitian", "Accountant", "Auditor", "Bookkeeper", "Bookkeeper, Self Employed", "Book-keeper", "Dentist's Assistant", "Editor", "Electrologist", "Inspector", "Lawyer", "Manuscript Reader", "Map Corrector"),
    "Pink Collar" = c("[Assistant] Housekeeper", "Teacher", "hairdresser", "clerk","cook", "laundress", "Manicurist", "Teacher", "Luncheonette", "Kitchen [woman]", "Instructor", "Hatter", "Dressmaker", "Dress Maker", "Dipper", "Decorator", "City Missionary", "Church Works", "Chocolate Dipper", "Candy worker", "Candy Dipper", "Stenographer", "Tailoress", "Typist", "Waitress", "Nurse", "nurse", "Trained Nurse", "trained nurse", "Lodging house-keeper", "Lodging house keeper", "Lodging House", "Lodging house", "librarian", "Librarian", "Coffee maker", "Clerical Work", "Clerk", "Clerk - office work", "Chambermaid", "cashier", "Cashier", "Carpet-sweeper", "Assistant", "Bank clerk", "Bank Clerk", "Office cleaning", "steno", "Sten.", "Bank Teller", "bank teller", "Bank teller", "Clerk & Cashier", "Clerk, Not employed", "Cost Clerk", "File Clerk", "Invoice Clerk", "Janitress", "Laundress", "Music Teacher", "Pastry Cook", "Physical Instructor", "Private Secretary", "Sales-clerk", "Saleslady", "School Teacher", "Seamstress", "Secretary", "Secretary, Not employed", "secretary", "Stenographer", "Stenographer, Not employed", "Stock Clerk", "Storekeeper", "Chamber maid, married", "Counting clerk", "[Lodging House]", "[Secretary]", "[Taileress]", "Attendant", "attendant", "Designer", "Drawing teacher", "dishwasher", "Ins. clerk", "Insurance clerk", "Interior decorator", "Kitchen"),
    "Blue Collar" = c("Forelady", "forewoman", "Lamp Inspector", "Factory work", "Factory Work", "Packer", "Assembler", "Factory Operator", "Forewoman", "General Worker", "Mach. Operator", "Machine Operator", "Mill Hand", "Mill work", "Mill Worker", "Operator", "Rubind Mach. Feeder", "Sealing & Operating", "Spinner", "Spooler", "Metal worker", "milliner", "Milliner", "millinery"),
    "Housewife" = c("At home", "at home", "At home", "At Home, Housekeeper", "At Home, Modiste", "At Home, Widow", "housewife", "Housewife", "Housewife, At Home", "housewife, married", "Housewife, Married", "Married, housewife", "Married, Housewife", "None, At Home", "house-wife", "Married None", "married housewife", "Married Housewife", "domestic", "Domestic", "[Home]", "[Housework]"),
...
bwvcollars <- bwvclean %>%
  filter(occupation_category == "White Collar" | occupation_category =="Blue Collar" | occupation_category == "Pink Collar" | occupation_category == "Student" | occupation_category == "Housewife") %>%
  arrange(Ward, occupation_category)

#I also ran the following code to get the average age of each of my categories across wards.
bwvcollars %>% group_by(occupation_category) %>% summarize(mean_age = mean(mean_value))
  
```

(@) What did you discover? Write 2-3 sentences about what this tells us about Boston women voters.

> First off, in Ward 2, there were only white collar workers and housewives who voted. Wards 1, 4-11, 13, 16, 18, and 19 were the only wards with blue collar workers who voted. Students who voted were located in wards 1, 6, 8, 10, 11, and 13. Housewife voters were the oldest group across all wards, with an average age of 43.1 years. Student voters were the youngest group with an average age of 22.5 years. I didn't clean up the data entirely, so some professions may be omitted that would change this observation, but the categories I used were pretty broad and I think I got a lot of the data cleaned up and sorted into them.
